...
@register.inclusion_tag('facebook/show_string.html',takes_context=True)
def show_facebook_last_name(context,user):
    if isinstance(user,FacebookProfile):
        p = user
    else:
        p = user.facebook_profile
    if getattr(settings,'WIDGET_MODE',None):
        #if we're rendering widgets, link direct to facebook
        return {'string':u'<fb:name uid="%s" lastnameonly="true" />' % (p.facebook_id)}
    else:
        return {'string':u'<a href="%s">%s</a>' % (p.get_absolute_url(),p.last_name)}

@register.inclusion_tag('facebook/show_string.html',takes_context=True)
def show_facebook_full_name(context,user):
    if isinstance(user,FacebookProfile):
        p = user
    else:
        p = user.facebook_profile
    if getattr(settings,'WIDGET_MODE',None):
        ...
        return {'string':u'%s' % (p.full_name)}

@register.inclusion_tag('facebook/show_string.html',takes_context=True)
def show_facebook_large_photo(context,user):
    if isinstance(user,FacebookProfile):
        p = user
    else:
        p = user.facebook_profile
    if getattr(settings,'WIDGET_MODE',None):
        ...
        return {'string':u'<a href="%s"><img src="%s" alt="%s"/></a>' % (p.get_absolute_url(), p.picture_large_url, p.full_name)}
...
@register.inclusion_tag('facebook/movies.html')
def show_friend_movies(friendIds):
	fb = get_facebook_client()
	facebook_uid = fb.uid
	movies = []
	...
	for i in friendIds:
		...
		tmovies = json.dumps( curmovielist )
		tmovies = json.loads( tmovies )
		movies.append( tmovies[0]['movies'] )
	movies.sort()
	ret = []
	arrlen = len( movies )
	i = 0
	while i < arrlen:
		j = i
		ret.append( movies )
		while j < arrlen and movies[i] == movies[j]:
			j = j+1
			i = j
	return {'movies': ret }   
